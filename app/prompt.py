...
## 최우선 원칙 (반드시 지킬 것)
"목표 어법"이 지정되더라도, 그것은 아래 1~5번 전체 분석 규칙에 "추가되는" 요구사항일 뿐입니다.
목표 어법 문장만 골라서 표시하고 나머지 분석(어휘 태그, 문법 태그, 번역, 사이드 노트 등)을
생략하거나 대충 하는 것은 절대 금지입니다. 지문의 모든 문장에 대해 1~5번 규칙을 빠짐없이
동일한 밀도로 적용한 다음, 그 위에 목표 어법에 해당하는 문장만 추가로 표시하세요.
...
- 밀도: 문법/어휘적으로 설명할 가치가 있는 단어는 거의 다 태그. **문장당 최소 2개 이상의
  ...
  단어는 태그하지 않되, 그 대신 다른 단어에서 최소 개수를 채우세요.
- 이 태깅 밀도 요구사항은 목표 어법 지정 여부와 절대 무관합니다. 목표 어법에 신경 쓰느라
  다른 문장의 어휘("v") 태그를 줄이는 것은 규칙 위반입니다. 최종 출력 전에 각 문장의
  tag 개수를 스스로 세어보고, 2개 미만인 문장이 있으면 어휘 태그를 추가해서 채우세요.
...
(예: "가정법 도치, 분사구문"). 이건 "이 문장들만 분석해줘"가 아니라
"전체 분석에 더해서, 이 문장들에는 표시와 설명을 추가로 얹어줘"라는 뜻입니다.
...
- 목표 어법과 무관한 다른 모든 문장에도 1, 3, 4번 규칙(어휘 태그, 번역, 사이드 노트)을
  절대 생략하지 말고 동일하게 적용하세요.
...
### 4. 사이드 노트 (notes)
문장마다 **최소 1개, 최대 3개**, 실제로 해당하는 카테고리만 선택 (0개인 문장이 있으면 안 됨):
...
notes[].category는 반드시 위 6개 값 중 하나여야 합니다. "target"은 badge 필드에만 쓰는
값이며 notes[].category에는 절대 사용하지 마세요 (목표 어법 문장의 어법 설명은
"grammar" 카테고리 노트로 작성하세요).
목표 어법 문장이 아니어도 "comprehension" 노트는 기본으로 채우세요 — 목표 어법에
집중하느라 다른 문장의 노트를 비우는 것은 규칙 위반입니다.
